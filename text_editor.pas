...
begin
  ...
  if RichEdit1.Lines.Count<>0 then
                              begin
                                buttonSelected := MessageDlg('Сохранить файл перед открытием нового?',mtConfirmation, mbOKCancel, 0);
                                if buttonSelected=1 then
                                                    begin
                                                      ...
                                                      if opendialog1.filename <>'' then Form1.Caption:='Текстовый редактор [ '+ opendialog1.filename+' ]';
                                                    end
...
begin
  form2.visible:=false;
  if savedialog1.Execute then richedit1.Lines.SaveToFile(savedialog1.filename);
  Form1.Caption:='Текстовый редактор [ '+savedialog1.filename+' ]';
  form2.visible:=true;
  Form1.SetFocus;
end;
...
begin
  if RichEdit1.Lines.Count<>0 then
                              begin
                                buttonSelected := MessageDlg('Сохранить изменения перед выходом?',mtConfirmation, mbOKCancel, 0);
                                if buttonSelected=1 then
                                                    begin
                                                      if savedialog1.Execute then richedit1.Lines.SaveToFile(savedialog1.filename);
                                                      Form1.Close;
...
begin
  MessageDlg('Создатель:'+#10+'> Миргаязов Камиль'+#10+'> студент 2 курса'+#10+'> группы 7291-11',mtInformation, [mbOK], 0);
end;
...
begin
  if RichEdit1.Lines.Count<>0 then
                              begin
                                buttonSelected := MessageDlg('Сохранить файл перед созданием нового?',mtConfirmation, mbOKCancel, 0);
                                if buttonSelected=1 then
                                                    begin
                                                      if savedialog1.Execute then richedit1.Lines.SaveToFile(savedialog1.filename);
                                                      RichEdit1.Lines.Clear;
  ...
  Form1.Caption:='Текстовый редактор [ новый файл ]';
end;
